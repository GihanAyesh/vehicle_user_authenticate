...
class MTCNN():
    ...
    def align_multi(self, img, limit=None):
        ...
        for landmark in landmarks:
            facial5points = np.array([[landmark[j], landmark[j + 5]] for j in range(5)]).astype(np.float32)
            ref_pts = np.array(self.refrence).astype(np.float32)

            # ---- Compute similarity transform ----
            M, _ = cv2.estimateAffinePartial2D(facial5points, ref_pts)
            ...
            orig_coords = cv2.transform(np.array([corners]), M_inv)[0]

            aligned_coords.append(orig_coords)

        return boxes, faces, aligned_coords
    ####################
